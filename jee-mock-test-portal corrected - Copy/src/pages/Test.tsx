...
import { Clock, CheckCircle, XCircle, RotateCcw, User, LogOut, AlertTriangle, Shield, BookOpen, Menu, Trophy, BarChart3 } from 'lucide-react';
...
export default function Test() {
  ...
  useEffect(() => {
    ...
    const preventShortcuts = (e: KeyboardEvent) => {
      ...
      if (mobileScreenshotKeys.some(check => check())) {
        const currentTime = Date.now();
        if (currentTime - lastVolumeKeyTime < 500) {
          volumeKeyPressCount++;
        } else {
        ...
        if (volumeKeyPressCount >= 2 || e.key === 'Power') {
          e.preventDefault();
          e.stopPropagation();
          e.stopImmediatePropagation();
          showViolationWarning('Mobile screenshot attempt detected - VIOLATION RECORDED');
          setScreenshotBlocked(true);
    ...
    document.addEventListener('visibilitychange', handleVisualChange);

    return () => {
      document.removeEventListener('keydown', preventShortcuts, true);
      document.removeEventListener('keyup', preventShortcuts, true);
      document.removeEventListener('visibilitychange', handleVisualChange);
    };
  }, [testStarted]);
